    ...
    public Polynomial(TreeMap<Integer, Fraction> coef) {
        for(Map.Entry<Integer, Fraction> e: coef.entrySet()){
            if(!e.getValue().isZero()){
                this.coef.put(e.getKey(), e.getValue());
            }
        }
    }

    public Polynomial(Integer[] pow, Fraction[] coef) {
        for(int i = 0; i < pow.length; i++){
            if(coef[i].sus != 0) {
                this.coef.put(pow[i], coef[i]);
            }
        }
    }
//
...
//    }
    ...
    String monomialToString(Map.Entry<Integer, Fraction> en, boolean firstTerm){
        String retStr = "";
        if(en.getValue().isNegative()){
            if(en.getValue().sus == -1  && en.getValue().jos == 1) {
                retStr += "-x";
            }
            else {
                retStr += en.getValue() + "x";
            }
        }
        else {
            if(!firstTerm) {
    ...
    public String toString() {
        ...
        if(it.hasNext()){
            ...
            if(en.getKey() == 0){
                ...
                retStr += monomialToString(en, true);
            }
        }
        ...
        while(it.hasNext()){
            ...
            if(en.getKey() == 0){
                ...
                retStr += monomialToString(en, false);
            }
        }
